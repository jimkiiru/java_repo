...
<title>Reports Upload | DeKUT Clubs Manager</title>
...
<div class="panel panel-info">
	<div class="panel-heading">
		<h2 class="text-left">Uploaded Reports</h2>
	</div>
	<center>
		<form action="searchReportsChair.do" method="post">
			<table class="table">
				<tr>
					<td><input class="input-search" type="text" placeholder="Search Report by club name"name="clubid" size="25"/> <input
						type="submit" class="btn btn-danger" value="Search" /> &nbsp;&nbsp;
						<input type="button" value="New Report" class="btn btn-primary col-lg-offset-9" onclick="javascript:go('saveReportChair.do');" />
					</td>
				</tr>
			</table>
		</form>
		<table class="table">
			...
			<c:if test="${empty SEARCH_CONTACTS_RESULTS_KEY}">
				<tr>
					<td colspan="4"><font style="color:red;">No Results found</font></td>
				</tr>
			</c:if>
			<c:if test="${! empty SEARCH_CONTACTS_RESULTS_KEY}">
				<c:forEach var="report" items="${SEARCH_CONTACTS_RESULTS_KEY}">
					<tr>
						<td><c:out value="${report.count}"></c:out></td>
						<td><c:out value="${report.uploaddate}"></c:out></td>
						<td><c:out  value="${report.url}"></c:out></td>
						<td>&nbsp;<a href="updateReportChair.do?count=${report.count}">Edit</a>
							&nbsp;&nbsp;<a  href="javascript:deleteReport('deleteReportChair.do?count=${report.count}');">Delete</a>
